...
function FeatureInfoUrlTemplateMixin(Base) {
    class FeatureInfoUrlTemplateMixin extends Base {
         ...
         */
        async getFeaturesFromPickResult(screenPosition, pickResult, loadExternal = true) {
            const feature = await this.buildFeatureFromPickResult(screenPosition, pickResult);
        ...
        wrapImageryPickFeatures(imageryProvider) {
            ...
            const proxiedFeatureInfoUrl = proxyCatalogItemUrl(this, featureInfoUrlTemplate !== null && featureInfoUrlTemplate !== void 0 ? featureInfoUrlTemplate : "", "0d");
            ...
            imageryProvider.pickFeatures = async (x, y, level, longitude, latitude) => {
                ...
                if (isDefined(featureInfoUrlTemplate) &&
                    isDefined(features) &&
                    ...
                    for (let i = 0; i < features.length; i++) {
                        ...
                        try {
                            ...
                            // Update description of the feature after it is resolved from
                            // feature info template url
                            feature.description = generateCesiumInfoHTMLFromProperties(feature.properties, currentTime !== null && currentTime !== void 0 ? currentTime : JulianDate.now(), showStringIfPropertyValueIsNull);
                        }
                        catch (_e) {
                            if (!feature.properties) {
                                feature.properties = {};
