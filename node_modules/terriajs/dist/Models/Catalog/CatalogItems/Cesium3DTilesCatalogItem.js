...
class Cesium3DTilesCatalogItem extends SearchableItemMixin(FeatureInfoUrlTemplateMixin(Cesium3dTilesMixin(CreateModel(Cesium3DTilesCatalogItemTraits)))) {
    ...
    highlightFeaturesFromItemSearchResults(results) {
        ...
        const disposeWatch = this._watchForNewTileFeatures(tileset, async (feature) => {
            ...
            if (resultIds.has(featureId)) {
                ...
                if (results.length === 1) {
                    disposeFeatureInfoPanel = await openInfoPanelForFeature(this, feature, SEARCH_RESULT_TAG);
                }
            }
        });
 ...
 */
const openInfoPanelForFeature = action(async (item, cesium3DTileFeature, excludePropertyFromPanel) => {
    var _a;
