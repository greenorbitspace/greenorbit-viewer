...
export default class Cesium extends GlobeOrMap {
    ...
    doZoomTo(target, flightDurationSeconds = 3.0) {
        ...
        const _zoom = async () => {
            ...
            else if (defined(target.imageryProvider) &&
     ...
     *
     */
    async pickFromScreenPosition(screenPosition, ignoreSplitter) {
     ...
     */
    async pickVectorFeatures(screenPosition) {
        var _a, _b;
        ...
        for (let i = 0; i < pickedList.length; ++i) {
            ...
            if (FeatureInfoUrlTemplateMixin.isMixedInto(catalogItem) &&
                ...
                const result = await catalogItem.getFeaturesFromPickResult.bind(catalogItem)(screenPosition, picked, vectorFeatures.length < catalogItem.maxRequests);
                if (result) {
    ...
    _buildPickedFeatures(providerCoords, pickPosition, existingFeatures, featurePromises, defaultHeight, ignoreSplitter) {
        ...
        result.allFeaturesAvailablePromise = Promise.all(featurePromises)
            ...
            runInAction(() => {
                ...
                result.features = allFeatures.reduce((resultFeaturesSoFar, imageryLayerFeatures, _i) => {
    ...
    _addVectorTileHighlight(imageryProvider, _rectangle) {
